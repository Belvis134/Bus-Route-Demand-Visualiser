    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          selectInput("default_key", "Select one of the default keys", choices = c("Default key 1" = "default_key_1", "Default key 2" = "default_key_2"), width = "500px")
        ),
        conditionalPanel(
      ...
      actionButton("import_routes", "Import Routes", width = "140px", icon = icon("table")),
      htmlOutput("result_conf"),
      htmlOutput("result_conf2")
    ),
    mainPanel(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$data1_in, {
    ...
    if (!is.null(pre_data1()) && !is.null(data_type1())) {
      conf_msg(paste("<span style='color:#00DD00; font-weight:bold;'><i class='fas fa-square-check'></i> Datamall ",
                     ...
                     else if (grepl("^transport_node_bus_", input$data1_in$name)) "Bus tap-in/out"
                     else if (grepl("^transport_node_train_", input$data1_in$name)) "Train tap-in/out",
  ...
  datamall_params <- eventReactive(input$import_datamall, {
    if (input$use_own_key) {account_key <- input$own_key}
    else {account_key <- input$default_key}
    list(account_key = account_key)
  })
  
  datamall_params2 <- eventReactive(input$import_routes, {
    if (input$use_own_key) {account_key <- input$own_key}
    else {account_key <- input$default_key}
    list(account_key = account_key)
  })
    ...
    if ("by_bus_svc" %in% heatmap_type || "by_mrt_line" %in% heatmap_type) {
      ...
      if (is.null(stop_cur)) {
        ...
        if (heatmap_data_type == "od_bus" || heatmap_data_type == "spec_bus") {
          fail("Invalid bus service. Is your bus service withdrawn?")
        } else {
      ...
      img <- Heatmap(dataod1c,
       ...
       cell_fun = function(j, i, x, y, width, height, fill) {
         ...
         } else if(dataod1c[i,j] >= 1 && input$show_below_30) {
           grid.text(sprintf("%.0f", dataod1c[i, j]), x, y, gp = gpar(fontsize = 14))
         }
       },
      ...
      if (heatmap_type == "by_specific_stops" || heatmap_type == "by_specific_stns") {
        ...
        if (l_ori != l_dst) {
          fail("The lengths of your origin bus stops/stations and destination bus stops/stations do not match.")
        }
      }
